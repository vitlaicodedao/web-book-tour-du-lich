...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $psaleprice = $_POST['packagesaleprice'];
        $pfeatures = $_POST['packagefeatures'];
        $pdetails = $_POST['packagedetails'];
        $pimage = $_FILES["packageimage"]["name"];
        move_uploaded_file($_FILES["packageimage"]["tmp_name"], "pacakgeimages/" . $_FILES["packageimage"]["name"]);
        $sql = "INSERT INTO goidulich(tengoi,kieugoi,diadiem,giatien,giagiam,phuongtien,mota,anh) VALUES(:pname,:ptype,:plocation,:pprice,:psaleprice,:pfeatures,:pdetails,:pimage)";
        $query = $dbh->prepare($sql);
        $query->bindParam(':pname', $pname, PDO::PARAM_STR);
        $query->bindParam(':ptype', $ptype, PDO::PARAM_STR);
        $query->bindParam(':plocation', $plocation, PDO::PARAM_STR);
        $query->bindParam(':pprice', $pprice, PDO::PARAM_STR);
        $query->bindParam(':psaleprice', $psaleprice, PDO::PARAM_STR);
        ...
        if ($lastInsertId) {
            $msg = "Tạo gói du lịch thành công.";
        } else {
            $error = "Đã xả ra lỗi. Vui lòng thử lại.";
        }
    }
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="horizontal-form">
                                <form class="form-horizontal" name="package" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
                                        <label for="focusedinput" class="col-sm-2 control-label">Giá giảm</label>
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control1" name="packagesaleprice"
                                                id="packagesaleprice" placeholder=" Giá giảm (VND)" required>
                                        </div>
                                    </div>
